...
public class SalesTeamReport implements CustomReport {
    ...
    private static final Logger LOGGER = getLogger(SalesTeamReport.class);
    ...
    private void signReport(Date dateThreshold, RecordType mode) {
        String sb = "generated by " +
                AppBeans.get(UserSessionSource.class).getUserSession().getUser().getName() +
                " at " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd.MM.yy HH:mm", Locale.US)) +
                " (mode: \"" + AppBeans.get(Messages.class).getMessage(mode) + '"' +
