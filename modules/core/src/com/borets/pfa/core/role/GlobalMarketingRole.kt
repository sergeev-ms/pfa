...
class GlobalMarketingRole : AnnotatedRoleDefinition() {
    companion object {
        const val NAME = "global-marketing"
        const val DESCRIPTION = "Responsible for global marketing strategy"
    }
    ...
    @EntityAccessContainer(
        ...
        EntityAccess(entityClass = MarketData::class, operations = [EntityOp.READ, EntityOp.CREATE, EntityOp.UPDATE]),
        ...
        EntityAccess(entityClass = Attachment::class,
            operations = [EntityOp.READ, EntityOp.UPDATE, EntityOp.CREATE, EntityOp.DELETE]
        ),
        EntityAccess(entityClass = Supplementary::class, operations = [EntityOp.READ]),
        EntityAccess(entityClass = SupplementaryDetail::class,
            operations = [EntityOp.READ, EntityOp.CREATE, EntityOp.UPDATE]
        ),
        EntityAccess(entityClass = SupplementaryDetailType::class,
    ...
    @EntityAttributeAccessContainer(
        EntityAttributeAccess(entityClass = Account::class,
            view = ["*"],
            modify = ["actualMarketDetail", "marketDetails", "supplementary"]
        ),
        EntityAttributeAccess(entityClass = MarketData::class, modify = ["*"]),
        ...
        EntityAttributeAccess(entityClass = Attachment::class, modify = ["*"]),
    ...
    override fun getLocName(): String {
        return "Global Marketing Strategy"
    }
